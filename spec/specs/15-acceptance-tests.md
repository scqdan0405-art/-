...
## E14b. 認証・アカウント管理（16）
- [ ] must_change_password の店舗アカウント → 変更完了まで /store の全画面へ進めない。11文字パスワードは拒否。
- [ ] ban 済み店舗アカウント → 既存セッションでも次のAPIから拒否。
- [ ] 失効スタッフコード → 403 + audit記録。再有効化後は成功。

## E14c. OTP不達の回避策（06）
- [ ] 受取OTP発行後10分以内に `/b/[token]` を開く → `activePickupOtp` が表示され、そのOTPで checkout 成功。期限切れ/使用後は非表示。
- [ ] 預入前にメール修正（PATCH email）→ 新旧両アドレスへ通知、以後のメールは新アドレス宛。stored 後の修正は 409。
